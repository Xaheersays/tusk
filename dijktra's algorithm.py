...
# 1. if src and destination are the same then dist is 0. if Dist is an array [] where Dist[i] will store the best found shortest distance from src to i. We write.
...
# 2. if dist[x] = dx & dis[y] = dy and we discover edge[x][y] = p ( meaning wt of edge from x to y is p ) such that 
...
# 	from start we know only for src we have an absolute shortest path which is Dist[Src] = 0 and it can not be updated since all edges have +ve weight.
